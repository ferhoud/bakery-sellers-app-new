...
function upsertSeller(map, sellerIdValue, fullNameValue = "", { active = true, force = false } = {}) {
  ...
  map.set(sellerId, {
    ...current,
    full_name: fullName && current.full_name === current.seller_id ? fullName : current.full_name || fullName || sellerId,
    active: current.active !== false || active !== false,
    has_month_activity: current.has_month_activity || !!force,
  });
}

function addHours(map, sellerId, hours) {
  const id = String(sellerId || "").trim();
  if (!id) return;
  const h = Number(hours || 0) || 0;
  map.set(id, round2((map.get(id) || 0) + h));
}

function addMinutes(map, sellerId, minutes) {
  const id = String(sellerId || "").trim();
  if (!id) return;
  const m = Math.round(Number(minutes || 0) || 0);
...
export async function loadPayrollAdjustmentPreview(service, monthValue) {
  ...
  const [
    ...
    maybeSelect(service, "sellers", "id, full_name, active"),
    maybeSelect(service, "profiles", "user_id, full_name, role, active"),
    maybeSelect(service, "payroll_adjustment_settings", "*"),
    maybeSelect(service, "payroll_adjustment_monthly", "*", (q) => q.eq("payroll_month", month.payroll_month)),
  ...
  // Les deux tables de sauvegarde du module doivent exister.
  // Si elles n'existent pas, il faut exécuter le SQL du module.
  if (settingsResult.error) throw settingsResult.error;
  if (monthlyResult.error) throw monthlyResult.error;

  const sellerById = new Map();

  // 1) Sources fiables de vendeuses actives.
  (listSellersRpc.data || []).forEach((row) => {
    const sellerId = sellerIdFromRow(row);
    const fullName = sellerNameFromRow(row);
    const active = row?.active !== false;
    upsertSeller(sellerById, sellerId, fullName, { active });
  });

  (activeNamesRpc.data || []).forEach((row) => {
    const sellerId = sellerIdFromRow(row);
    const fullName = sellerNameFromRow(row);
    upsertSeller(sellerById, sellerId, fullName, { active: true });
  });

  (sellersResult.data || []).forEach((row) => {
    const sellerId = sellerIdFromRow(row);
    const fullName = sellerNameFromRow(row);
    upsertSeller(sellerById, sellerId, fullName, { active: row?.active !== false });
  });
  ...
  (profilesResult.data || []).forEach((row) => {
    ...
    // On garde les profils vendeuses, mais on accepte aussi les profils déjà présents par une autre source.
    if (role === "seller" || sellerById.has(sellerId)) {
      upsertSeller(sellerById, sellerId, fullName, { active });
    }
  });
  ...
  // 2) Heures planning via RPC si disponible, car c'est la logique admin existante.
  (hoursRpc.data || []).forEach((row) => {
    ...
    if (!sellerId) return;
    upsertSeller(sellerById, sellerId, fullName, { active: true, force: true });
    addHours(planningHoursBySeller, sellerId, hours);
  });

  // 3) Fallback shifts si RPC absente ou incomplète.
  if (planningHoursBySeller.size === 0) {
    (shiftsResult.data || []).forEach((row) => {
      ...
      addHours(planningHoursBySeller, sellerId, h);
      upsertSeller(sellerById, sellerId, "", { active: true, force: true });
    });
  }

  // 4) Fallback attestations mensuelles si les shifts historiques ne sont plus lisibles.
  (attestationsResult.data || []).forEach((row) => {
    ...
    if (!planningHoursBySeller.has(sellerId) || Number(planningHoursBySeller.get(sellerId) || 0) === 0) {
      const hours = Number(row?.final_hours ?? row?.computed_hours ?? 0) || 0;
      if (hours) planningHoursBySeller.set(sellerId, round2(hours));
    }
  });

  // 5) Toute vendeuse avec une configuration/saisie doit apparaître.
  (settingsResult.data || []).forEach((row) => {
    upsertSeller(sellerById, row?.seller_id, "", { active: true, force: true });
  });
  ...
  (extraResult.data || []).forEach((row) => {
    ...
    addMinutes(extraMinutesBySeller, sellerId, minutes);
  ...
  const rows = Array.from(sellerById.values())
    .filter((seller) => {
      // On garde les vendeuses actives et celles qui ont une activité/saisie sur le mois.
      return seller?.active !== false || seller?.has_month_activity === true;
    })
    .map((seller) => {
      ...
      const paidLeaveDays =
        saved?.paid_leave_days_override != null
          ? round2(saved.paid_leave_days_override)
          : autoPaidLeaveDays;
      ...
      const payslipHours = round2(
        saved?.payslip_hours ??
          setting?.default_payslip_hours ??
          0
      );
