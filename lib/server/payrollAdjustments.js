...
export async function loadPayrollAdjustmentPreview(serviceInput, monthValue) {
  ...
  const rows = Array.from(sellerMap.values())
    ...
    .map((seller) => {
      ...
      const payslipHoursEntered =
        saved?.payslip_hours != null || setting?.default_payslip_hours != null;
      const payslipHours = payslipHoursEntered
        ? round2(saved?.payslip_hours ?? setting?.default_payslip_hours ?? 0)
        : 0;

      const hourlyRate = round2(saved?.hourly_rate ?? setting?.hourly_rate ?? DEFAULT_HOURLY_RATE);

      const hoursDifference = payslipHoursEntered ? round2(totalDueHours - payslipHours) : null;
      const complementHours = payslipHoursEntered ? round2(Math.max(0, hoursDifference)) : null;
      const complementAmount = payslipHoursEntered ? round2(complementHours * hourlyRate) : null;
      ...
      return {
        ...
        payslip_hours_entered: payslipHoursEntered,
  ...
  const rowsWithPayslipHours = rows.filter((r) => r.payslip_hours_entered);
  ...
  return {
    ...
    summary: {
      ...
      payslip_hours_entered_count: rowsWithPayslipHours.length,
      ...
      total_payslip_hours: round2(rowsWithPayslipHours.reduce((sum, r) => sum + (Number(r.payslip_hours) || 0), 0)),
      total_complement_hours: round2(rowsWithPayslipHours.reduce((sum, r) => sum + (Number(r.complement_hours) || 0), 0)),
      total_complement_amount: round2(rowsWithPayslipHours.reduce((sum, r) => sum + (Number(r.complement_amount) || 0), 0)),
    },
    debug_counts: {
