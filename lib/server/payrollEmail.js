...
function publicHolidayMapFrance(year) {
  const easter = easterDateUtc(Number(year));
  const rows = [
    { iso: `${year}-01-01`, label: "1er janvier" },
    { iso: isoFromUtcDate(addDaysUtcDate(easter, 1)), label: "lundi de Pâques" },
    { iso: `${year}-05-01`, label: "1er mai" },
    { iso: `${year}-05-08`, label: "8 mai" },
    { iso: isoFromUtcDate(addDaysUtcDate(easter, 39)), label: "Ascension" },
    { iso: isoFromUtcDate(addDaysUtcDate(easter, 50)), label: "lundi de Pentecôte" },
    { iso: `${year}-07-14`, label: "14 juillet" },
    { iso: `${year}-08-15`, label: "15 août" },
    { iso: `${year}-11-01`, label: "Toussaint" },
    { iso: `${year}-11-11`, label: "11 novembre" },
    { iso: `${year}-12-25`, label: "Noël" },
  ];

  return new Map(rows.map((row) => [row.iso, row.label]));
}

function holidayDatesForPayrollMonth(month) {
  const year = Number(month?.year || 0);
  if (!year) return [];
  const map = publicHolidayMapFrance(year);
  const start = String(month?.start_iso || "").slice(0, 10);
  const end = String(month?.end_iso || "").slice(0, 10);
  return Array.from(map.keys()).filter((iso) => iso >= start && iso <= end).sort();
}

function fmtMinutesPayroll(mins) {
  const n = Math.max(0, Math.round(Number(mins || 0) || 0));
...
function extraWorkHolidayLabel(row) {
  const kind = String(row?.kind || "").toLowerCase();
  const reason = normalizeLoose(row?.reason || row?.notes || "");

  if (kind === "coverage") return "couverture";
  if (kind === "relay") return "relai";
  if (reason.includes("renfort")) return "renfort";
  if (reason.includes("couverture")) return "couverture";
  if (reason.includes("relai") || reason.includes("relais")) return "relai";
  return "travail en plus";
}

export function buildHolidayWorkNote({ sellerId, plannedRows, extraRows, holidayByDate }) {
  const sid = String(sellerId || "").trim();
  if (!sid) return "";

  const map = new Map();

  const ensure = (dateIso) => {
    const d = String(dateIso || "").slice(0, 10);
    if (!d || !holidayByDate?.has?.(d)) return null;
    if (!map.has(d)) {
      map.set(d, {
        date: d,
        label: holidayByDate.get(d) || formatDayMonthFr(d),
        planned: false,
        extraMinutes: 0,
        extraKinds: new Set(),
      });
  ...
  (plannedRows || [])
    .filter((row) => String(row?.seller_id || "") === sid)
    .forEach((row) => {
      const entry = ensure(row?.date || row?.work_date);
      if (entry) entry.planned = true;
    });
  ...
  (extraRows || [])
    ...
    .forEach((row) => {
      ...
      if (minutes <= 0) return;
      const entry = ensure(row?.work_date || row?.date);
      if (!entry) return;
      entry.extraMinutes += minutes;
      entry.extraKinds.add(extraWorkHolidayLabel(row));
    });

  const entries = Array.from(map.values()).sort((a, b) => a.date.localeCompare(b.date));
  if (!entries.length) return "";

  const parts = entries.map((entry) => {
    const dateLabel = entry.label || formatDayMonthFr(entry.date);
    if (entry.extraMinutes > 0) {
      const kinds = Array.from(entry.extraKinds || []).filter(Boolean);
      const kindText = kinds.length ? formatFrenchList(kinds) : "travail en plus";
      return `${fmtMinutesPayroll(entry.extraMinutes)} en ${kindText} le ${dateLabel}`;
    }
    return `le ${dateLabel}`;
  });

  if (parts.length === 1) {
    return `a travaillé ${parts[0]} (jour férié)`;
  }

  return `a travaillé les jours fériés suivants : ${formatFrenchList(parts)}`;
}
...
export async function buildPayrollEmailPreview(admin, monthValue) {
  ...
  const holidayDates = holidayDatesForPayrollMonth(month);
  const holidayByDate = publicHolidayMapFrance(month.year);
  let holidayShiftRows = [];
  let holidayExtraRows = [];
  ...
  if (holidayDates.length > 0) {
    ...
    holidayExtraRows = Array.isArray(extraRows) ? extraRows : [];
  }
  ...
  const holidayPlanningRows = Array.isArray(holidayShiftRows) ? holidayShiftRows : [];
  const holidayWorkRows = Array.isArray(holidayExtraRows) ? holidayExtraRows : [];
  ...
  const enrichedEmployees = monthlyEmployees.map((employee) => {
    ...
    const holidayWorkNote = buildHolidayWorkNote({
      sellerId,
      plannedRows: holidayPlanningRows,
      extraRows: holidayWorkRows,
      holidayByDate,
    });
  ...
  return {
    ...
    summary: {
      ...
      public_holiday_dates_count: holidayDates.length,
      holiday_planning_rows_count: holidayPlanningRows.length,
      holiday_extra_work_rows_count: holidayWorkRows.length,
      employees_with_holiday_work_notes_count: holidayWorkNoteCount,
    },
  };
}
