// touch: 2025-10-08 v-admin-cancel-direct+banner + weekly-totals-pastdays
...
function TotalsGrid({
  ...
  // Heures semaine — **semaine EN COURS seulement**, et **uniquement les jours déjà passés** (exclut aujourd’hui et futurs)
  const weekTotals = useMemo(() => {
    ...
    const isoDaysPast = weekDays.map(d => fmtISODate(d)).filter(d => d < todayIso);

    Object.keys(SHIFT_HOURS).forEach((code) => {
      isoDaysPast.forEach((iso) => {
        const sellerId = assign[`${iso}|${code}`];
        if (sellerId) dict[sellerId] = (dict[sellerId] || 0) + (SHIFT_HOURS[code] || 0);
      });
    });
    return dict;
  }, [sellers, assign, refreshKey]);
